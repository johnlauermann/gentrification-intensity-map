...
here::i_am("01_data/data_combineallyears_2020tr.r")
...
decadal_change <- function(data, variables, year1, year2) {
  for (v in variables) {
    ...
    data[[v_change]] <- data[[v_year2]] - data[[v_year1]]
  }
...
data <- decadal_change(data = data, variables = variable_list, year1 = 1990, year2 = 2020)
...
data <- data %>% select(tr2020gj, sort(setdiff(names(.), "tr2020gj")))
write.csv(data, file = "tractdata_allyears_2020tr.csv", na="", row.names = FALSE)

##save a sample of 'urban' census tracts
boundaries <- read.csv(here("03_spatial/metrotracts_2020tr.csv"))
data <- boundaries %>%
